...
class UNetBatchGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(input_files)):
            ...
            input_slice = input_data[i].numpy() * 255 # Extract the 2D patch for input
            label_slice = output_data[i].numpy() * 255 # Extract the 2D patch for labels

            # Ensure the directory for this batch exists
            create_directory_structure(f"./running_data2d/batch{idx}")

            cv2.imwrite(f"./running_data2d/batch{idx}/{count}slice{i}_input.png", input_slice.astype(np.uint8))
            cv2.imwrite(f"./running_data2d/batch{idx}/{count}slice{i}_labels.png", label_slice.astype(np.uint8))
    ...
    def create_dataset(self):
        ...
        def process_npy_file(input_file, label_file):
            ...
            input_data.set_shape([128, 128, 1])  # Assuming input is 128x128 grayscale images depth 5
            label_data.set_shape([128, 128, 3])  # Assuming label is 128x128 with 3 classes depth 5
