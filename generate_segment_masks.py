...
def save_preprocessed_training_data2d(csv_file_path='./data/train.csv', output_directory='./preprocessed_data2d', width=256, height=256):
    input_dir = f"{output_directory}/input_data"
    label_dir = f"{output_directory}/labels"
    i = 1  # Use numeric_prefix for simplicity

    logging.info("Starting 2D data preprocessing")
    logging.info(f"CSV File Path: {csv_file_path}")
    logging.info(f"Output Directory: {output_directory}")
    
    # Assuming parser.load_data_as_volume loads data in format (input_images, label_images, sample_id)
    # input_images: shape (num_samples, width, height)
    # label_images: shape (num_samples, width, height, num_channels)
    for input_images, label_images, sample_id in parser.load_data_as_volume(csv_file_path, width=width, height=height):
        
        # Iterate over the first dimension (batch dimension)
        num_samples = input_images.shape[0]
        logging.info(f"input shape: {input_images.shape}")
        ...
        for j in range(num_samples):
            ...
            # Extract the j-th input and label image
            input_image = np.asarray(input_images[j]).reshape(width, height, 1)  # Shape should be (256, 256, 1)
            label_image = np.asarray(label_images[j]).reshape(width, height, 3)  # Shape should be (256, 256, 3)
            
            # Save the 2D images as individual samples
            input_file_path = os.path.join(sample_input_dir, f"image_{j+1}.npy")
            label_file_path = os.path.join(sample_label_dir, f"label_{j+1}.npy")

            # Save input and label as .npy files
            np.save(input_file_path, input_image.astype(np.float32))
            np.save(label_file_path, label_image.astype(np.float32))

            # Log the saving process
            logging.debug(f"Saved input image to: {input_file_path}")
            logging.debug(f"Saved label image to: {label_file_path}")
            
        i += 1

    logging.info("2D data preprocessing complete.")
...
def save_preprocessed_training_data3d(csv_file_path='./data/train.csv', output_directory='./preprocessed_data3d', depth=5, stride=3):
    ...
    for input, labels, sample_id in parser.load_data_as_volume(csv_file_path, width=128, height=128):
        sample_input_dir = os.path.join(input_dir,f"{i}_{sample_id}")
        sample_label_dir = os.path.join(label_dir,f"{i}_{sample_id}")
...
if __name__ == "__main__":
	...
	save_preprocessed_training_data2d(width=128,height=128)
	# save_preprocessed_training_data3d()
