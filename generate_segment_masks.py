...
def save_preprocessed_training_data3d(csv_file_path='./data/train.csv', output_directory='./preprocessed_data3d', depth=5, stride=3):
    ...
    for input, labels, sample_id in parser.load_data3d(csv_file_path):
        ...
        for j in range(num_patches):
            ...
            input_file_path = f"{input_dir}/{i}_{sample_id}_patch{j+1}.npy"
            label_file_path = f"{label_dir}/{i}_{sample_id}_patch{j+1}.npy"
            ...
            logging.info(f"Saved input patch to: {input_file_path}")
            logging.info(f"Saved label patch to: {label_file_path}")
